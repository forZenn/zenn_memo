...
こうすると/usr/local/binにnodeとnpmのシンボリックリンクを置いておくといい感じに
やってくれます。

```bash
sudo ln -s /usr/local/node/${version}/bin/node  /usr/local/bin/node
sudo ln -s /usr/local/node/${version}/bin/npm  /usr/local/bin/npm
sudo ln -s /usr/local/node/${version}/bin/npx  /usr/local/bin/npx
```
...
#!/usr/bin/env bash
# 
# nodejs /usr/local/node/\$version version management
...
# Example:
#   nodelocals list
#   # out put installed version from /usr/local/node/\$version.
#   nodelocals local \$version
#   # set node, npm, npx to /usr/local/bin
#######################################
function nodelocals() {
  ...
  local version
  # nodeがない場合は空文字
  local selected_version=$(readlink /usr/local/bin/node | awk  -F/ '{print $5}')
  for ((i=0;i<$#;i++)); do
    # if find list flags from args, show installed nodejs destination /usr/local/node.
    if [ "${new_array[$i]}" = "list" ]; then
      ...
      local selected
      for version in $(ls -1 /usr/local/node); do
	      selected=" "
    ...
    if [ "${new_array[$i]}" = "local" ]; then
      ...
      local exist=1
      # versionの存在確認
      for check_version in $(ls -1 /usr/local/node); do
      ...
      # symlinkで紐付け
      ln -snf /usr/local/node/${version}/bin/node  /usr/local/bin/node
      ln -snf /usr/local/node/${version}/bin/npm   /usr/local/bin/npm
      ln -snf /usr/local/node/${version}/bin/npx   /usr/local/bin/npx
      return 0
    fi
  done
}
...
function usage() {
    cat 1>&2 <<EOF
nodelocals
nodejs /usr/local/node/\${version} allignment version management tools

USAGE:
    nodelocals [FLAGS] [OPTIONS]

FLAGS:
    list                    Show /usr/local/node/\$version installed nodejs version engines.
    local                   Set node, npm, npx to /usr/local/bin
...
```bash
# bin
ls -1 /usr/local/node/${version}/bin
# lib
ls -1 /usr/local/node/${version}/lib
```
