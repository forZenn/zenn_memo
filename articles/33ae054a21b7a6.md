...
```bash
# こうすると時間経過もログに残せる。
make -j4 2>&1 | awk '{print strftime("%Y-%m-%dT%H:%M:%S"), $0}' | tee ~/var/log/node/make/"$(date +'%Y%m%dT%H%M%SZ')".log
```
...
iostat -x 1 | awk '{print strftime("%Y-%m-%dT%H:%M:%S"), $0}' | tee ~/var/log/iostat/"$(date +'%Y%m%dT%H%M%SZ')".log
```
