...
zenn-cliがnodejs12以下はサポート切ってたりで、bullseyeのnpmからzenn-cliをインストール使用とすると怒られます。一応入りますが、警告だが、非推奨だかのメッセージが出ます。

よってraspberry piからapt->npm->zenn-cliという感じに怒られないええ感じにインストールできない。
単純にnodejs関連のパッケージだけstable bookwormのレポジトリを向くようにしてもいいですが、それでも最新のLTSでないし、どうせ最新のLTSでないならまあせっかく?なんでビルドしましょうと思ってやってみました。
...
ビルド方法はnodejsの場合はBUIILDING.mdに書いてあります。
大抵のプロジェクトにはローカルのビルド方法が書いてあるので、その通りにやってください。
無い場合はソースコードを読んだり、実際ビルドしたときにエラーが出るlibファイルを見たり、Makefile読んだりで自分で調べましょう。
古いプロジェクトでない限り、githubにやり方が書かれていたりして、How to的な環境は整っていると思います。
nodejsはかなり丁寧なので、わかりやすいのです。

### 依存関係

Debianは下のようなパッケージが必要です。
...
tmuxで起動
これはsshでやってたりして、処理が中断されないようにするためです。
...
```bash
make -j4 2>&1 | tee ~/var/log/node/make/"$(date +'%Y%m%d%H%M%S')".log
```
...
# これをやると1秒間に
iostat -x 1 | awk '{print strftime("%Y-%m-%d%H%M%S"), $0}' | tee ~/var/log/stat/io/"$(date +'%Y%m%d%H%M%S')".log
```
...
raspberry pi4Bでビルドしたところ、3時間から4時間ほどで
...
負荷を少しでも下げたい場合、raspberrypi でやっている人はGUIを切ってやった方がいいかも。
私はraspberry pi4Bで最初にビルドした場合はクラッシュしたのでsystemctl set-default multi-user.targetとしてcuiにしてビルドしなおしました。

これらの数値はあくまで私の環境での話であり、高温、低温下や、microsdカードの空き容量や種類によるストレージの速度などでも速度は変わると思います。
参考までに。

ただ、最新のPCなら30分から１時間で終わりそうななのであまり気にしなくていい気もします。
...
make installで上書きという形だと前のバージョンの依存関係のライブラリが残ります。
気をつけましょう。
