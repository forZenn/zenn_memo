...
```bash
dnf install --installroot=/var/lib/machines/opensuse/ --releasever 15.3 --repo opensuse-leap rpm-repos-openSUSE-Leap systemd systemd-network systemd-resolved bash zypper util-linux
```

zypper addrepo -f 

```bash
zypper ar -f -c http://download.opensuse.org/distribution/leap/15.3/repo/oss repo-oss
zypper ar -f -c http://download.opensuse.org/distribution/leap/15.3/repo/non-oss repo-non-oss
```
